...
#creating NACL +
resource "aws_network_acl" "nginx_nacl" {
    depends_on = ["aws_instance.nginx_inst1"]
    vpc_id = "${aws_vpc.nginx_vpc.id}"
    subnet_ids = ["${aws_subnet.nginx_public_subnet1.id}"]
    egress {
        protocol = "-1"
        rule_no = 100
        action = "allow"
...
resource "aws_route_table" "nginx_route" {
    vpc_id = "${aws_vpc.nginx_vpc.id}"
    route {
...
resource "aws_route_table_association" "nginx_route_table_association1" {
  subnet_id      = "${aws_subnet.nginx_public_subnet1.id}"
  route_table_id = "${aws_route_table.nginx_route.id}"
}
...
resource "aws_elb" "nginx_elb" {
    name = "nginx-elb"
    subnets         = ["${aws_subnet.nginx_public_subnet1.id}"]
    security_groups = ["${aws_security_group.nginx_security_group.id}"]
    instances       = ["${aws_instance.nginx_inst1.id}"]
    cross_zone_load_balancing   = true
 #    access_logs {
 #        # bucket        = "nginx_S3_logging"
 # #       bucket_prefix = "bar"
 #        interval      = 5
 #    }
    listener {
...
#creating INSTANCE1
resource "aws_instance" "nginx_inst1" {
    key_name = "${var.key_name}"
    connection = {
        user = "ubuntu"
...
output "inst_ip" {
    value = "${aws_instance.nginx_inst1.public_ip}"
}
...
#  instance = "${aws_instance.nginx_inst1.id}"
#}

#output "ip" {
#  value = "${aws_eip.nginx_ip.public_ip}"
#}